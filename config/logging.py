...
class TelegramHttpxFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        """Filter out HTTP request logs from httpx/httpcore."""
        if record.name.startswith(("httpx", "httpcore")) and "HTTP Request:" in record.getMessage():
            return False
        msg = record.getMessage()
        if (
            "HTTP Request:" in msg
            and "api.telegram.org" in msg
            and (match := self._pattern.search(msg))
        ):
